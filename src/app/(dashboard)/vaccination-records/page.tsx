...
export const metadata = {
  title: "Vaccination Records — SyncVet",
  description:
    "Anti-rabies and parvovirus vaccination tracking for the CDO Veterinary Office.",
};
...
const metrics: PageMetric[] = [
  {
    ...
    sub: "Anti-Rabies / Parvo / Deworm",
  },
  {
    title: "Anti-Rabies (2026)",
    value: "4,812",
    icon: ShieldCheck,
    ...
    sub: "Canine & Feline doses",
  },
  {
    title: "Overdue Boosters",
    value: "342",
    icon: AlertTriangle,
  ...
  return (
    <div className="flex-1 space-y-4 p-6 pt-6 bg-background min-h-screen text-foreground">
      <PageHeader
        ...
        subtitle={
          <>
            Anti-rabies, anti-parvovirus, and deworming dose tracking with{" "}
            <span className="text-foreground">batch traceability</span> and booster scheduling.
          </>
        }
        actions={
          <>
      ...
      <Card>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <CardDescription className="text-[11px] text-muted-foreground/60">
                All administered doses and overdue boosters across CDO barangays
              </CardDescription>
            </div>
            <div className="flex items-center gap-3">
              <div className="relative">
                <Search className="absolute left-2.5 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" />
                <Input placeholder="Search by pet, batch, or vet..." className="h-8 w-64 pl-8 text-xs" />
              </div>
        ...
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="text-[10px] font-bold uppercase tracking-widest">Vax ID</TableHead>
            ...
            <TableBody>
              {records.map((r) => (
                <TableRow key={r.id} className="hover:bg-muted/50">
                  ...
                  <TableCell>
                    <div>
                      <p className="text-sm font-semibold">{r.petName}</p>
                      <p className="text-[10px] text-muted-foreground">{r.petId}</p>
                    </div>
                  </TableCell>
